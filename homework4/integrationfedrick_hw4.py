...
#everything with whole adds all the bins together and computes the total integral from a to b using
...
def rhomberg(N,a,b,steps,function):
    L=2**N
    h=(float(b)-(a))/N
    A=zeros(2**N)
    B=zeros(2**N,1)
    ...
    for x in range(0,L):
        for y in range(0,L):
            if y==0:
                ...
                B[x,0]=twhole(int(n),a,b,function)[2]
...
def rhombergwhole(a,b,step,error,function):
    ...
    for i in range(0,len(bins)-1):
        ...
        N=1 
        while( not exitwhile):
            ans=rhomberg(N,locala,localb,step,function)
